...
for rc in $(find "$HOME/.zsh" -maxdepth 1 -name '*.*sh' -type f | sort -n)
do
    source $rc
done
...
for rc in $(find "$HOME/.zsh" -name '*.*sh' -type f | sort -n)
do
    source $rc
done

# Automatically start tmux session on ssh
if [[ -n "$PS1" ]] && [[ -z "$TMUX" ]] && [[ -n "$SSH_CONNECTION" ]] && [[ ( "$LC_TERMINAL" == "iTerm2" ) || ( -z "$LC_TERMINAL" ) ]] && which tmux >> /dev/null
then
